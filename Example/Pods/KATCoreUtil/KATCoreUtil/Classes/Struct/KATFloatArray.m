...
{
    KATFloatArray *array=[[[self alloc] init] autorelease];
    
    //初始化
    [array initData:FLOAT_ARRAY_CAPACITY_DEFAULT];
...
- (void)initData:(int)capacity
{
    ...
    //给成员分配空间
    _member=(double *)malloc(sizeof(double)*capacity);
    
    //初始成员变量赋空值
    for(int i=0;i<_capacity;i++)
    {
        _member[i]=FLOAT_ARRAY_VALUE_EMPTY;
    }
}
...
//            }
            ...
            //拷贝内存
            memcpy(_member, members, sizeof(double)*_length);
...
- (double)get:(int)index
{
    ...
    {
        return FLOAT_ARRAY_VALUE_EMPTY;
    }
}
...
- (BOOL)set:(double)value withIndex:(int)index
{
    ...
    if(index>=0 && index<_length)
    {
        if(value==FLOAT_ARRAY_VALUE_EMPTY)
        {
...
{
    if(index<0 || index>_length)//容量已满或者index不在length范围内，则插入失败
    {
        return NO;
    }
...
- (KATFloatArray *)getFormRange:(NSRange)range;
{
    ...
    {
        KATFloatArray *array=[KATFloatArray floatArrayWithCapacity:(int)range.length];
...
- (BOOL)deleteWithIndex:(int)index
{
    ...
    if(index>=0 && index<_length)
    {
        ...
        _member[_length-1]=FLOAT_ARRAY_VALUE_EMPTY;//最后的成员赋空值
...
- (BOOL)deleteFromRange:(NSRange)range
{
    ...
    if((range.location-1+range.length)<_length)
    {
        ...
        {
            _member[i]=FLOAT_ARRAY_VALUE_EMPTY;//最后的成员赋空值
        }
...
- (void)forwardByStep:(int)step
{
    if(_length>0)
    {
        ...
        if(step>0)
        {
            double *tmp=(double *)malloc(sizeof(id)*step);//临时变量数组
...
- (void)sort
{
    floatQuickSort(_member,0,_length-1);
    //    floatBubbleSort(_member,_length);
}
...
//快速排序
void floatQuickSort(double arr[],int s,int e)
{
    if(s<e)
    {
        int m=floatPartion(arr,s,e);//交换，得到中间位置
        
        floatQuickSort(arr,s,m-1);//左边部分排序
        
        floatQuickSort(arr,m+1,e);//右边部分排序
    }
}


int floatPartion(double arr[],int s,int e)
{
    //rand
...
//冒泡排序
void floatBubbleSort(double arr[],int length)
{
    BOOL flag=NO;
...
{
    NSMutableString *ms=[NSMutableString stringWithFormat:@"KATFloatArray: cap=%i, len=%i, sum=%.4lf, max=%.4lf, min=%.4lf\n{\n",_capacity,_length,self.sum,self.max,self.min];
    
    for(int i=0;i<_length;i++)
    {
        [ms appendFormat:@"   [%i] %lf \n",i,_member[i]];
    }
...
{
    KATFloatArray *array=[[[self class] allocWithZone:zone] init];
...
//    NSLog(@"KATFloatArray is dealloc");
